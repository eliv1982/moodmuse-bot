...
def test_text_handlers_use_cleanup() -> None:
    assert "_cleanup_pending_field_confirmation" in inspect.getsource(main.on_image_description)
    assert "_cleanup_pending_field_confirmation" in inspect.getsource(main.on_holiday)
...
async def test_new_voice_cleans_up_previous_pending() -> None:
    ...
    async def _offer_with_cleanup(*args: object, **kwargs: object) -> None:
        await main._cleanup_pending_field_confirmation(args[0], args[2])  # type: ignore[arg-type]

    with (
        patch.object(main, "_cleanup_pending_field_confirmation", cleanup),
        patch.object(main, "_transcribe_voice_message", AsyncMock(return_value="new")),
        patch.object(main, "_safe_delete_message", AsyncMock()),
        patch.object(main, "_offer_field_text_confirm", AsyncMock(side_effect=_offer_with_cleanup)),
    ):
        await main._transcribe_and_offer_voice_confirm(
            MagicMock(), message, state, "ru", MagicMock(), field=VOICE_FIELD_IMAGE
        )
...
async def test_cleanup_deletes_confirm_and_source() -> None:
    ...
    with patch.object(main, "_safe_delete_message", delete):
        snapshot = await main._cleanup_pending_field_confirmation(bot, state)
...
async def test_cleanup_delete_failure_does_not_crash() -> None:
    ...
    await main._cleanup_pending_field_confirmation(bot, state)
    state.update_data.assert_awaited()
...
async def test_confirm_deletes_source_not_confirm_before_finalize() -> None:
    ...
    with (
        ...
        patch.object(main, "_cleanup_pending_field_confirmation", cleanup),
        patch.object(main, "_apply_confirmed_image", AsyncMock()),
    ):
...
async def test_type_instead_deletes_messages() -> None:
    ...
    with (
        patch.object(main, "_lang_from_state", AsyncMock(return_value="ru")),
        patch.object(main, "_cleanup_pending_field_confirmation", cleanup),
        patch.object(main, "_reprompt_field", AsyncMock()),
    ):
        await main.on_voice_confirm_action(cq, state, MagicMock())
...
@pytest.mark.asyncio
async def test_typed_text_offers_confirmation() -> None:
    state = AsyncMock()
